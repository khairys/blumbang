    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="flex justify-end mb-8">
            <form action="{{ route('publik.potensi-jagung.index') }}" method="GET" class="relative w-full md:w-64">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari konten..." class="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <article class="bg-white rounded-2xl overflow-hidden border border-emerald-100 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-200 flex flex-col">
                <a href="{{ route('publik.potensi-jagung.show', $p->slug) }}" class="block relative aspect-video">
                    ...
                    <div class="absolute top-3 right-3">
                        <span class="text-xs font-bold bg-white/95 text-emerald-700 px-3 py-1 rounded-full shadow-sm">{{ $p->category }}</span>
                    </div>
                </a>
                <div class="p-5 flex-1 flex flex-col">
                    <p class="text-xs text-gray-400 mb-2">{{ $p->created_at->format('d M Y') }}</p>
                    <h3 class="font-bold text-gray-900 mb-2 leading-snug"><a href="{{ route('publik.potensi-jagung.show', $p->slug) }}" class="hover:text-emerald-600 transition-colors">{{ $p->title }}</a></h3>
                    <p class="text-sm text-gray-500 leading-relaxed line-clamp-3 mb-4">{{ $p->summary }}</p>
                    <div class="mt-auto pt-4 border-t border-gray-50">
                        <a href="{{ route('publik.potensi-jagung.show', $p->slug) }}" class="inline-flex items-center gap-1.5 text-emerald-600 hover:text-emerald-700 text-sm font-semibold transition-colors">
                            Baca Edukasi
        ...
        <div class="py-16 text-center bg-emerald-50 rounded-2xl border border-emerald-100">
            <div class="mb-4 flex justify-center text-emerald-300"><svg class="w-16 h-16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg></div>
