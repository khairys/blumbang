<x-publik-layout>
    ...
    <div class="bg-gradient-to-br from-emerald-800 via-emerald-700 to-green-600 pt-16 pb-24 relative overflow-hidden">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <h1 class="text-4xl font-extrabold text-white tracking-tight">Hubungi Kami</h1>
            <p class="text-emerald-50 mt-3 text-lg max-w-2xl">Informasi kontak dan lokasi Kantor Desa Blumbang</p>
        </div>
        <!-- Wave -->
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex gap-3">
                            <div class="w-10 h-10 bg-emerald-100 rounded-xl flex items-center justify-center flex-shrink-0 text-emerald-700"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg></div>
                            <div>
                                <p class="text-sm font-semibold text-gray-700 mb-1">Telepon</p>
                                <a href="tel:{{ $pengaturan->phone ?? '#' }}" class="text-sm text-emerald-600 hover:underline">{{ $pengaturan->phone ?? 'Belum diatur' }}</a>
                            </div>
                        </div>
                        <div class="flex gap-3">
                            ...
                            <div>
                                ...
                                <a href="mailto:{{ $pengaturan->email ?? '#' }}" class="text-sm text-emerald-600 hover:underline">{{ $pengaturan->email ?? 'Belum diatur' }}</a>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <!-- Medsos -->
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    <h2 class="font-bold text-gray-900 mb-4 flex items-center gap-2"><span class="text-emerald-600"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg></span> Media Sosial</h2>
                    <div class="flex flex-wrap gap-3">
                        @if(isset($pengaturan) && $pengaturan->facebook)
                        <a href="{{ $pengaturan->facebook }}" target="_blank" class="flex items-center gap-2.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 text-sm font-medium px-4 py-2.5 rounded-xl transition-colors">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                            Facebook
                        </a>
                        @endif
                        @if(isset($pengaturan) && $pengaturan->instagram)
                        <a href="{{ $pengaturan->instagram }}" target="_blank" class="flex items-center gap-2.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 text-sm font-medium px-4 py-2.5 rounded-xl transition-colors">
