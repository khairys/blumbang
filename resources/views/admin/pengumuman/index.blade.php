<x-app-layout>
    <x-slot name="header">
        <h1 class="text-lg font-bold text-gray-900">Manajemen Pengumuman</h1>
    </x-slot>
    <div class="p-6 space-y-5">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-5 border-b border-gray-100">
                <div>
                    <h2 class="font-bold text-gray-900">Daftar Pengumuman</h2>
                    <p class="text-sm text-gray-500 mt-0.5">Total: {{ $pengumumans->total() }} pengumuman</p>
                </div>
                <div class="flex items-center gap-2">
                    <form action="{{ route('admin.pengumuman.index') }}" method="GET" class="relative">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari pengumuman..." class="pl-9 pr-4 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-400">
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                    <tr>
                        ...
                        <th class="text-left px-4 py-3 hidden md:table-cell">Berlaku Hingga</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-4 py-3 text-gray-500 text-xs hidden md:table-cell">
                            @if($item->expired_at)
                                @php $expired = \Carbon\Carbon::parse($item->expired_at) @endphp
                                <span class="{{ $expired->isPast() ? 'text-red-500' : 'text-gray-500' }}">
                                    {{ $expired->format('d M Y') }}
                                    {{ $expired->isPast() ? '(Expired)' : '' }}
                                </span>
                            @else — @endif
                        </td>
            ...
            <div class="px-4 py-3 border-t border-gray-100">{{ $pengumumans->links() }}</div>
            @endif
        </div>
    </div>
</x-app-layout>