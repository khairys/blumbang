<x-app-layout>
    <x-slot name="header"><h1 class="text-lg font-bold text-gray-900">Kategori Berita</h1></x-slot>
    <div class="p-6 space-y-5">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                ...
                <form method="POST" action="{{ route("admin.kategori-berita.store") }}">
                    @csrf
                    <div class="mb-4">
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                        <tr>
                            ...
                            <th class="text-center px-4 py-3">Jumlah Berita</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors" x-data="{ editing: false }">
                            ...
                            <td class="px-4 py-3 text-center" x-show="!editing">
                                <span class="bg-blue-100 text-blue-700 text-xs font-bold px-2.5 py-1 rounded-full">{{ $item->beritas_count }}</span>
                            </td>
                            <td class="px-4 py-3 text-right" x-show="!editing">
                                <div class="inline-flex items-center gap-1">
                                    ...
                                    @if($item->beritas_count == 0)
                                    <form method="POST" action="{{ route("admin.kategori-berita.destroy", $item) }}" onsubmit="return confirm("Hapus kategori ini?")">
                                        @csrf @method("DELETE")
                            ...
                            <td colspan="4" class="px-4 py-3" x-show="editing" x-cloak>
                                <form method="POST" action="{{ route("admin.kategori-berita.update", $item) }}" class="flex items-center gap-2">
                                    @csrf @method("PUT")
                                    <input type="text" name="name" value="{{ $item->name }}" required class="flex-1 px-3 py-1.5 border border-blue-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
                        ...
                        @empty
                        <tr><td colspan="4" class="px-4 py-12 text-center text-gray-400">Belum ada kategori. Tambahkan di form kiri.</td></tr>
                        @endforelse
                    </tbody>
                </table>
