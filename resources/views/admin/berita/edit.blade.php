                    ...
                    <div class="lg:col-span-2 space-y-5">
                        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="title" value="{{ old('title', $berita->title) }}" required class="w-full rounded-xl border-gray-300 text-sm">
                                @error('title') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Ringkasan</label>
                                <textarea name="summary" rows="3" class="w-full rounded-xl border-gray-300 text-sm">{{ old('summary', $berita->summary) }}</textarea>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Isi Berita <span class="text-red-500">*</span></label>
                                <textarea name="content" rows="10" required class="w-full rounded-xl border-gray-300 text-sm">{{ old('content', $berita->content) }}</textarea>
                        ...
                        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
                            ...
                            <div>
                                ...
                                <select name="category_id" class="w-full rounded-xl border-gray-300 text-sm">
                                    ...
                                    <option value="{{ $k->id }}" {{ $berita->category_id == $k->id ? 'selected' : '' }}>{{ $k->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            @if($berita->thumbnail)
                            <div>
                                ...
                                <img src="{{ Storage::url($berita->thumbnail) }}" class="w-full h-32 object-cover rounded-lg">
                            </div>
                            @endif
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Ganti Thumbnail</label>
                                <input type="file" name="thumbnail" accept="image/*" class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-green-50 file:text-green-700">
                            </div>
                        </div>
                        <div class="flex gap-3">
                            <button type="submit" class="flex-1 bg-green-600 hover:bg-green-700 text-white font-semibold py-2.5 rounded-xl transition-colors text-sm">Simpan Perubahan</button>
