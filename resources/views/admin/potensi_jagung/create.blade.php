<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Tambah Konten Jagung</h2>
        </div>
    </x-slot>
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @endif
            <form method="POST" action="{{ route('admin.potensi-jagung.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div class="lg:col-span-2">
                        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
                            <h3 class="font-bold text-gray-900 pb-2 border-b">Detail Konten</h3>
                <div>
                ...
                <div>
                    <label for="content" class="block text-sm font-medium text-gray-700 mb-1">Isi/Konten Lengkap </label>
...
<option value="published" {{ old('status') == 'published' ? 'selected' : '' }}>Dipublikasikan</option>
                    ...
                    @error('status') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Kategori Konten <span class="text-red-500">*</span></label>
                                <select name="category" required class="w-full rounded-xl border-gray-300 text-sm">
                                    <option value="">-- Pilih --</option>
                                    <option value="Potensi Jagung" {{ old('category') == 'Potensi Jagung' ? 'selected' : '' }}>Potensi Jagung</option>
                                    <option value="Kandungan Gizi" {{ old('category') == 'Kandungan Gizi' ? 'selected' : '' }}>Kandungan Gizi</option>
                                    <option value="Produk Olahan" {{ old('category') == 'Produk Olahan' ? 'selected' : '' }}>Produk Olahan</option>
