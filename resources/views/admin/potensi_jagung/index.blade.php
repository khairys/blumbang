<x-app-layout>
    <x-slot name="header">
        <h1 class="text-lg font-bold text-gray-900">Manajemen Produk Jagung & Stunting</h1>
    </x-slot>
    <div class="p-6 space-y-5">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-5 border-b border-gray-100">
                <div>
                    <h2 class="font-bold text-gray-900">Konten Jagung & Pencegahan Stunting</h2>
                    <p class="text-sm text-gray-500 mt-0.5">Total: {{ $produk->total() }} konten</p>
                </div>
                <div class="flex items-center gap-2">
                    <form action="{{ route('admin.potensi-jagung.index') }}" method="GET" class="relative">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari konten..." class="pl-9 pr-4 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-400">
                        <svg class="w-4 h-4 text-gray-400 absolute left-3 top-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                    </form>
                    <a href="{{ route('admin.potensi-jagung.create') }}"
                       class="inline-flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-colors shrink-0">
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-3">
                                    ...
                                    @else
                                    <div class="w-10 h-10 bg-amber-100 rounded-lg flex items-center justify-center text-lg flex-shrink-0">🌽</div>
                                    @endif
                                    <div>
                                        <p class="font-medium text-gray-900 line-clamp-1">{{ $item->title }}</p>
                                        @if($item->summary)<p class="text-xs text-gray-400 line-clamp-1 mt-0.5">{{ $item->summary }}</p>@endif
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-3 hidden md:table-cell">
                                <span class="text-xs bg-amber-50 text-amber-700 px-2 py-0.5 rounded-full">{{ $item->category }}</span>
                            </td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="inline-flex items-center gap-1">
                                    <a href="{{ route('admin.potensi-jagung.edit', $item) }}"
                                    ...
                                    <form method="POST" action="{{ route('admin.potensi-jagung.destroy', $item) }}" onsubmit="return confirm('Hapus konten ini?')">
                                        @csrf @method('DELETE')
                        ...
                        <tr><td colspan="4" class="px-4 py-16 text-center">
                            ...
                            <p class="text-gray-500 font-medium">Belum ada konten jagung</p>
                        </td></tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            @if($produk->hasPages())
            <div class="px-4 py-3 border-t border-gray-100">{{ $produk->links() }}</div>
            @endif
        </div>
    </div>
</x-app-layout>